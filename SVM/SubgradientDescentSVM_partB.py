...
def primalStochasticSubGradientDescent(df, T, C, gamma0):
    ...
    for e in range(1, T):     # Iterate RANDOMLY based on available examples
        ...
        for i in range(0,rows):
            ...
            gamma_t = gamma0 / (1 + i)              # Update gamma_t for every example
...
WLearned_C1, ObjectiveVals1 = primalStochasticSubGradientDescent(biasedDFTrain, T, C[0], gamma0)
WLearned_C2, ObjectiveVals2 = primalStochasticSubGradientDescent(biasedDFTrain, T, C[1], gamma0)
WLearned_C3, ObjectiveVals3 = primalStochasticSubGradientDescent(biasedDFTrain, T, C[2], gamma0)
...
plt.title('Objective Function at $\gamma_0 = $' + str(gamma0) + ' (without a)')
